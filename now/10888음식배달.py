...
프렌차이즈에서는 “배달 거리”라는 말을 사용한다. 배달 거리는 집과 가장 가까운 음식배달집 사이의 거리이다. 즉, 배달 거리는 집을 기준으로 정해지며, 음식배달집이 결정되면 각각의 집은 배달 거리가 결정된다.
...
for t in range(1, T+1):
    ...
    dic = {}
    for i in range(n):
        for jdx, j in enumerate(list(map(int, input().split()))):
            if j == 1:
                house[(i,jdx)] = float('inf')
            elif j > 1:
                chicken.append((i,jdx))
                dic[(i,jdx)] = j

    m = float('inf')
    for i in range(1,len(chicken)+1):
        dis = 0
        for com in combinations(chicken,i):
            for co in com:
                dis += dic[co]
                for r,c in house.keys():
                    house[(r,c)] = min(house[(r,c)],abs(r-co[0]) + abs(c-co[1]))
        for r,c in house:
            dis += house[(r,c)]
            house[(r, c)] = float('inf')
